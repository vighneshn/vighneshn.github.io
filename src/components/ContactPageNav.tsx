...
// Navigation items for the five buttons
...
// Custom widths for each electrode (exactly matching the trap background 20% segments)
const buttonWidths = [
...
interface ContactPageNavProps {
  currentPage: string;
}

const ContactPageNav: React.FC<ContactPageNavProps> = ({ currentPage }) => {
  const navigate = useNavigate();
  
  // Filter out the current page
  const availableItems = navigationItems
    .filter(item => item.name.toLowerCase() !== currentPage.toLowerCase());
  
  // Determine the items we need to display
  const displayItems = [...availableItems];
  while (displayItems.length < 5) {
    displayItems.push({ name: '', path: '' }); // Empty placeholders if needed
  }
